...
class InstaSpiderUsers(scrapy.Spider):
    ...
    def __init__(self, login, enc_password, *args, **kwargs):
        self.insta_users = ['schekinaan', 'foxmalinart']
        self.login = login
    ...
    def url_user_followers(self, response, insta_user_data, variables=None):
        ...
        yield response.follow(url, callback=self.followers_parse, cb_kwargs={'user_data': insta_user_data})

    def followers_parse(self, response, insta_user_data):
        if b'application/json' in response.headers['Content-Type']:
            data = response.json()
            yield from self.get_follow_item(insta_user_data, data['data']['user']['edge_follow']['edges'])
    ...
    def get_follow_item(self, user_data, follow_users_data):
        for user in follow_users_data:
            yield InstaFollow(
                insta_user_id=user_data['id'],
                insta_user_name=user_data['username'],
